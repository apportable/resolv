	...
	*count = 0;
	...
	for (i = 0; i < dns->sdns->client_count; i++)
	{
		...
		for (j = 0; j < r->nscount; j++)
		{
			...
			{
#ifdef ANDROID
				if (memcmp(l[k], sa, sa->sa_family == AF_INET6 ? sizeof(struct sockaddr_in6) : sizeof(struct sockaddr_in)) == 0) found = 1;
#else
...
#ifdef ANDROID
			memcpy(l[n], sa, sa->sa_family == AF_INET6 ? sizeof(struct sockaddr_in6) : sizeof(struct sockaddr_in));
#else
